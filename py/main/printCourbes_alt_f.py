    ...
    for k in [ u for u in range(0,len(conditionCourbe)) if u != 1 ]:
        ...
        for i in range(len(tab)):
            ...
            if param == 0.002 or param == 0.005 or param == 0.01:
                ax.plot(t2, T2, pltColors[i], alpha=0.8, label = param*2,linewidth=10, path_effects=[outline])
            elif param == "carton":
                ax.plot(t2, T2, pltColors[i], alpha=0.8, label = "Cardboard",linewidth=10, path_effects=[outline])
            elif param == "cartonne":
                ax.plot(t2, T2, pltColors[i], alpha=0.8, label = "Hard paper",linewidth=10, path_effects=[outline])
            elif param == "rubleu":
                ax.plot(t2, T2, pltColors[i], alpha=0.8, label = "Imitation leather",linewidth=10, path_effects=[outline])
            elif param == "null":
                ()
                #ax.plot(t2, T2, pltColors[i], alpha=0.8, label = param)
            else:
                ax.plot(t2, T2, pltColors[i], alpha=0.8, label = param,linewidth=10, path_effects=[outline])
            bL = len(t2)
            if param != "null":
